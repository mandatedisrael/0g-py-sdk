#!/usr/bin/env python3
"""Compare the Python inference SDK with the upstream 0G Compute TS SDK.
...
DEFAULT_UPSTREAM_URL = "https://github.com/0gfoundation/0g-compute-ts-sdk.git"
DEFAULT_REF = "main"
DEFAULT_PYTHON_ROOT = "0g_py_inference/zerog_py_sdk"
DEFAULT_CACHE_DIR = ".cache/compute_ts_sdk"
DEFAULT_REPORT_MD = "reports/compute_parity_report.md"
DEFAULT_REPORT_JSON = "reports/compute_parity_report.json"
...
TS_SDK_HINTS = ("src.ts/sdk", "src.ts/example", "README.md", "Interface.md")
TS_PACKAGE_HINTS = (
    "src.ts/sdk",
    "src.ts/cli",
    "src.ts/example",
...
PYTHON_DOC_HINTS = ("0g_py_inference/README.md", "0g_py_inference/examples", "0g_py_inference/scripts")

FEATURE_PROBES = [
    {
        "id": "session_tokens",
        "label": "Session token auth",
        "scopes": ["sdk", "package"],
        "ts": [r"SessionToken", r"tokenId", r"Authorization.*Bearer app-sk"],
        "py": [r"SessionToken", r"token_id", r"Authorization.*Bearer app-sk"],
    },
    {
        "id": "persistent_api_keys",
        "label": "Persistent API keys",
        "scopes": ["sdk", "package"],
    ...
    },
    {
        ...
        "scopes": ["sdk", "package"],
        "ts": [r"verify.*Response", r"processResponse", r"attestation"],
        "py": [r"verify.*response", r"process_response", r"attestation"],
    },
    {
        "id": "read_only_broker",
        "label": "Read-only service browsing",
        "scopes": ["sdk", "package"],
        "ts": [r"ReadOnly.*Broker", r"listServiceWithDetail"],
        "py": [r"ReadOnly.*Broker", r"list_service_with_detail"],
    },
    {
        "id": "fine_tuning",
        "label": "Fine-tuning workflow",
        "scopes": ["sdk", "package"],
        "ts": [r"FineTuningBroker", r"createTask", r"uploadDataset"],
        "py": [r"FineTuningBroker", r"create_task", r"upload_dataset"],
    },
    {
        "id": "lora_adapters",
        "label": "LoRA adapter deployment",
        "scopes": ["sdk", "package"],
        "ts": [r"deployAdapter", r"listAdapters", r"adapterName"],
        "py": [r"deploy_adapter", r"list_adapters", r"adapter_name"],
    },
    {
        "id": "provider_controller",
        "label": "Provider controller tooling",
        "scopes": ["package"],
        "ts": [r"controller", r"provider.*controller", r"Controller"],
        "py": [r"controller", r"provider.*controller", r"Controller"],
    },
    {
        "id": "cli",
        "label": "CLI commands",
        "scopes": ["package"],
        "ts": [r"commander", r"\.command\(", r"0g-compute-cli"],
        "py": [r"argparse", r"click", r"console_scripts"],
    },
    {
        "id": "web_ui",
        "label": "Bundled web UI",
        "scopes": ["package"],
        "ts": [r"web-ui", r"web-ui-embedded", r"X402"],
        "py": [r"web-ui", r"web_ui", r"X402"],
    },
]
...
ACTION_DOMAIN_HINTS = {
    ...
    "lora_adapters": ["lora", "inference"],
    "provider_controller": ["cli", "shared"],
    "cli": ["cli"],
    "web_ui": ["web_ui"],
}
...
def domain_for_path(path: str) -> str:
    ...
    for prefix in ("0g_py_inference/zerog_py_sdk/", "zerog_py_sdk/"):
        if normalized.startswith(prefix):
            normalized = normalized[len(prefix) :]
            break
    if "/cli/" in normalized or normalized.startswith("src.ts/cli/"):
        return "cli"
    if "fine_tuning" in normalized or "fine-tuning" in path.lower():
        return "fine_tuning"
    ...
    if "session" in normalized or "auth" in normalized:
        return "auth_session"
    if "verifier" in normalized or "verify" in normalized:
        return "verification"
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--ts-path", type=Path, help="Local path to 0g-compute-ts-sdk. Skips remote fetch when provided.")
    parser.add_argument("--upstream-url", default=DEFAULT_UPSTREAM_URL, help="Git URL for the upstream TS SDK.")
    parser.add_argument("--ref", default=DEFAULT_REF, help="Branch, tag, or commit to compare. Defaults to main.")
    parser.add_argument("--cache-dir", type=Path, default=Path(DEFAULT_CACHE_DIR), help="Local checkout cache for the upstream TS SDK.")
    parser.add_argument("--python-root", type=Path, default=Path(DEFAULT_PYTHON_ROOT), help="Python inference SDK package root.")
    ...
    parser.add_argument(
        ...
        choices=("sdk", "package"),
        default="sdk",
        help="Compare only SDK-facing TS source by default, or the broader package including CLI/web UI.",
    )
...
def main(argv: list[str]) -> int:
    ...
    if not python_root.exists():
        raise FileNotFoundError(f"Python inference root does not exist: {python_root}")
